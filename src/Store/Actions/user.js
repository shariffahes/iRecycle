...
export const populateUserData = (id) => {
  return async (dispatch) => {
    ...
    if (userDataResponse.error?.message)
      _handleError(userDataResponse.error?.messag);
    dispatch({
      type: POPULATE_USER_DATA,
      userData: {
        userId: id,
        points: userDataResponse.points,
        location: userDataResponse.location,
        coupons: userDataResponse.coupons,
        name: userDataResponse.fullName,
        avatar: userDataResponse.avatar,
        accumulatedPoints: userDataResponse.accumulatedPoints,
      },
    });
  };
};
...
export const addPoints = (numberOfPoints) => {
  return async (dispatch, getState) => {
    fetch(baseFireBaseURL + `/users/${getState().user.userId}.json`, {
      method: "PATCH",
      body: JSON.stringify({
        points: getState().user.points + numberOfPoints,
        accumulatedPoints: getState().user.accumulatedPoints + numberOfPoints,
      }),
    })
...
export const addCoupon = (pointsClaimed, company, logo, title, description,discount) => {
  return async (dispatch, getState) => {
...
export const invalidateCoupon = (code) => {
  return async (dispatch, getState) => {
    const staleCoupons =
      getState().user.coupons.filter((coupon) => coupon.code !== code) ?? [];
    const freshCoupons = [
      ...staleCoupons,
      { code: code, invalid: true, expiryDate: "" },
    ];
